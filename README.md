...
For php `8.3`, `8.2` or `8.1`, select the image in `docker-compose.yml`. For older php versions, you need to download an [older version of php-sandbox](https://github.com/8ctopus/php-sandbox/releases/tag/1.2.8) and then choose the correct php version in `docker-compose.yml` as the architecture changed since.
